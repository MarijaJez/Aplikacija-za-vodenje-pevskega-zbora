...
% end
</div></details><a class="button secondary" href="/kategorije" data-permission="program">Uredi kategorije</a><button class="button primary" data-song-dialog data-create-dialog data-permission="program">＋ Dodaj pesem</button></div><div class="section-title"><div><strong>{{len(songs)}} pesmi</strong><span>v aktivnem programu</span></div></div><div class="songs-grid compact-song-grid" id="songs-grid">
% for song in songs:
...
% end
% if not songs:
<div class="empty-state card"><span>♫</span><h3>Program je še prazen</h3><p>Dodaj prvo pesem v program zbora.</p><button class="button primary" data-song-dialog data-permission="program">Dodaj pesem</button></div>
% end
